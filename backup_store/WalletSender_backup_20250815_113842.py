...
# ======== #MCP:IMPORTS ========
from typing import Callable, Any, TypeVar, Optional, Dict, List, Union, Tuple, Set
import sys
import os
import json
import time
...
# Унифицированная отправка raw-транзакций с fallback на оба имени метода
# (некоторые окружения/старые фрагменты кода могли использовать camelCase)
def send_raw_tx(w3, raw_tx_bytes):
    try:
        return w3.eth.send_raw_transaction(raw_tx_bytes)
    except AttributeError:
		# fallback для совместимости, если вдруг доступен camelCase
...
try:
    ...
    from web3 import middleware  # type: ignore
    # Совместимость с разными версиями Web3.py для газовых стратегий
    try:
        from web3.gas_strategies.time_based import medium_gas_price_strategy  # type: ignore
    except ImportError:
        try:
            from web3.gas_strategies.rpc import rpc_gas_price_strategy as medium_gas_price_strategy  # type: ignore
        except ImportError:
            medium_gas_price_strategy = None  # type: ignore
...
# Etherscan API for BSC network (новый Etherscan Multichain API)
BSCSCAN_URL   = "https://api.bscscan.com/api"
# SECURITY:MCP - Множественные API ключи для ротации при сбоях
BSCSCAN_KEYS  = [
    # Пользовательские ключи будут подставлены из config.json при старте

    "ARA9FYMNCIZHTB2PPBSWF686GID9F99P41",  # Основной ключ
    "ZZ3RSBZPMAPK4FV1HUVWE9X13G9ACJWPJX",  # Резервный ключ #1
    "ZV525F4QEKK2C3DWNGR69AUF6GG6Y2ZW39"   # Резервный ключ #2
]
BSCSCAN_KEY   = BSCSCAN_KEYS[0]  # Для обратной совместимости
...
def retry(times: int = 3, delay: float = 0.5) -> Callable[[F], F]:
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            raise RuntimeError("Все попытки не удались")
        return wrapper  # type: ignore
    return decorator

# ======== #MCP:CONFIG ========
class Config:
    # Добавим хранение Etherscan API ключей в конфиге (BSC сеть)
...
try:
    ...
    if _cfg_path.exists():
        ...
        user_keys = _data.get('bscscan_api_keys') or _data.get('bscscan_keys') or []
...
def add_history(token: str, to_addr: str, amt: float, tx_hash: str) -> None:
...
def add_found_transaction(tx_data: Dict[str, Any], search_info: Dict[str, Any]) -> None:
...
def add_reward(address: str, plex_amount: float, usdt_amount: float, tx_hash: Optional[str] = None) -> None:
...
def mark_transaction_rewarded(tx_hash: str) -> None:
...
def save_rewards_config(name: str, addresses: List[str], plex_amounts: Dict[int, float], usdt_amounts: Dict[int, float]) -> None:
...
def add_mass_distribution_item(distribution_id: int, address: str, cycle: int, tx_hash: str, status: str, error_message: Optional[str] = None) -> None:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.pk: Optional[str] = self.cfg.get_key()  # Получение приватного ключа из конфигурации
        self.mn: Optional[str] = self.cfg.get_mnemonic()  # Получение мнемонической фразы
        self.sending: bool = False  # Флаг активного процесса отправки
        self.tx_tracker: Optional[threading.Thread] = None  # Поток для отслеживания транзакций
        self.search_thread: Optional[threading.Thread] = None  # Поток для поиска транзакций
        self.is_searching: bool = False  # Флаг активного поиска
        self.stop_search_event = threading.Event()  # Событие для остановки поиска
        self.last_rewards_state: Optional[List] = None  # Хранение предыдущего состояния таблицы наград
...
• При блокировке всех ключей происходит автоматический сброс
    ...
    def _tab_found_tx(self):
        ...
        self.found_tx_table.horizontalHeader().setSectionResizeMode(6, QtWidgets.QHeaderView.Interactive)
...
• 🔄 Перезапустите приложение после установки
    ...
    def _update_address_status(self, row: int, status: str, color: str = None) -> None:
    ...
    def _send_bnb(self, to_address: str, amount: float) -> dict:
        """Отправка BNB"""
        try:
            w3 = self.rpc.web3()
    ...
    def _send_token(self, to_address: str, amount: float, token_address: str) -> dict:
        """Отправка токена"""
        try:
            w3 = self.rpc.web3()
            account = w3.eth.account.from_key(self.pk)
