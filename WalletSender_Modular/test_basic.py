...
def test_database():
    ...
    try:
        ...
        db.session.commit()
        print("   ✅ Тестовая награда создана")
        
        # Читаем награды
        rewards = db.session.query(Reward).all()
        print(f"   ✅ Найдено наград в БД: {len(rewards)}")
