...
def save_cleaned_addresses(addresses: List[str], output_file: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for addr in addresses:
            f.write(f"{addr}\n")
    print(f"✅ Сохранено {len(addresses)} адресов в {output_file}")
    ...
    for addr in addresses:
        ...
        for known_addr, description in known_addresses.items():
            if lower_addr == known_addr.lower():
                warnings.append(f"⚠️ {addr} - это {description}")
...
def main():
    """Главная функция"""
    print("=" * 60)
    print("🔍 Валидация адресов для рассылки")
    print("=" * 60)
    ...
    if warnings:
        print("\n⚠️ ВНИМАНИЕ! Обнаружены системные адреса:")
        ...
        print(f"\n📋 После фильтрации системных адресов: {len(eoa_addresses)} адресов")
