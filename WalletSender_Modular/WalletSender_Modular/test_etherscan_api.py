...
async def test_api():
    ...
    if block:
        ...
        print("[ERROR] Failed to get latest block")
    ...
    if balance is not None:
        print(f"[OK] Balance for {test_address[:10]}...: {balance} BNB")
    else:
    ...
    if token_balance is not None:
        print(f"[OK] USDT balance: {token_balance}")
    else:
    ...
    if block_by_time:
        print(f"[OK] Block at timestamp {timestamp}: {block_by_time}")
    else:
        print("[ERROR] Failed to get block by timestamp")
