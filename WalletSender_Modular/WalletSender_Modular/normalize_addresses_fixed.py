...
def normalize_addresses(input_file: str, output_file: str = None):
    ...
    # Обработка адресов
    valid_addresses = []  # Сохраняем порядок
    seen_addresses = set()  # Для проверки дубликатов
    invalid_lines = []
    duplicates = 0
    ...
    print(f"  [OK] Валидных адресов: {len(valid_addresses)}")
    print(f"  [WARN] Дубликатов удалено: {duplicates}")
    print(f"  [ERROR] Невалидных строк: {len(invalid_lines)}")
    print(f"\n[SAVE] Результат сохранен в: {output_path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Использование: python normalize_addresses.py <input_file> [output_file]")
        print("Пример: python normalize_addresses.py 01.09_ГЕНА_50.txt")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else None
    
    success = normalize_addresses(input_file, output_file)
