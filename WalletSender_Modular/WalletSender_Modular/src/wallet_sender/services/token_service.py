...
class TokenService:
    ...
    def transfer(
        ...
        Args:
            ...
            amount: Сумма токенов для отправки
            private_key: Приватный ключ отправителя
            gas_price: Цена газа в wei (опционально)
        ...
        for attempt in range(retry_count):
            try:
                ...
                # Проверка адресов
                if not self.web3.is_address(token_address):
                    return {'success': False, 'error': f'Неверный адрес токена: {token_address}'}
                    
                if not self.web3.is_address(to_address):
                    return {'success': False, 'error': f'Неверный адрес получателя: {to_address}'}
                ...
                if balance_wei < amount_wei:
                    return {'success': False, 'error': f'Недостаточный баланс токенов'}
                ...
                if gas_price is None:
                    gas_price = self.web3.eth.gas_price
                    # Добавляем 10% к текущей цене газа для ускорения
                ...
                if gas_limit is None:
                    ...
                    try:
                        ...
                        gas_limit = 100000  # Дефолтный лимит
                ...
                logger.info(f"Отправлена транзакция transfer: {tx_hash.hex()}")
                ...
                logger.error(f"Ошибка transfer (попытка {attempt + 1}/{retry_count}): {e}")
    ...
    def send_native(
        ...
        try:
            ...
            if balance < amount_wei:
                return {'success': False, 'error': 'Недостаточный баланс BNB'}
            ...
            if gas_limit is None:
                gas_limit = 21000  # Стандартный лимит для перевода BNB
            ...
            logger.info(f"Отправлена транзакция BNB: {tx_hash.hex()}")
