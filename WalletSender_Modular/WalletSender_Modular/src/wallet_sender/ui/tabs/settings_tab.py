...
from ...utils.logger_enhanced import (
    ...
    log_dialog_action, log_settings_change
)
...
class SettingsTab(BaseTab):
    ...
    def init_ui(self):
        ...
        # Заголовок
        header = QGroupBox("[SETTINGS] Настройки приложения")
        ...
        # Вкладка RPC
        self.tabs.addTab(self.create_rpc_tab(), "[WEB] RPC")
        ...
        # Вкладка токенов
        self.tabs.addTab(self.create_tokens_tab(), "[MONEY] Токены")
        
        # Вкладка API ключей
        self.tabs.addTab(self.create_api_tab(), "🔑 API")
        
        # Вкладка общих настроек
        self.tabs.addTab(self.create_general_tab(), "[INFO] Общее")
        
        # Вкладка обслуживания
        self.tabs.addTab(self.create_maintenance_tab(), "[CONFIG] Обслуживание")
        ...
        self.save_btn = QPushButton("[SAVE] Сохранить")
        ...
        self.import_btn = QPushButton("[SEND] Импорт")
    ...
    def create_rpc_tab(self) -> QWidget:
        ...
        # Тест основного RPC
        self.test_primary_btn = QPushButton("[TEST] Тест")
        ...
        # Тест резервного RPC
        self.test_fallback_btn = QPushButton("[TEST] Тест")
        ...
        self.chain_id.valueChanged.connect(lambda v: log_spinbox_change("Chain ID")(lambda: None)())
        ...
        # Тест всех RPC
        self.test_all_btn = QPushButton("[TEST] Тестировать все RPC")
    ...
    def create_gas_tab(self) -> QWidget:
        ...
        self.gas_price_gwei.valueChanged.connect(lambda v: log_spinbox_change("Цена газа")(lambda: None)())
        ...
        self.gas_limit_default.valueChanged.connect(lambda v: log_spinbox_change("Лимит газа")(lambda: None)())
    ...
    def create_api_tab(self) -> QWidget:
        ...
        # Тест API ключей
        self.test_api_btn = QPushButton("[TEST] Проверить ключи")
    ...
    def create_general_tab(self) -> QWidget:
        ...
        self.locale.currentTextChanged.connect(lambda t: log_dropdown_change("Язык")(lambda: None)())
        ...
        self.timezone.currentTextChanged.connect(lambda t: log_dropdown_change("Часовой пояс")(lambda: None)())
    ...
    def create_maintenance_tab(self) -> QWidget:
        """Создание вкладки обслуживания"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        # Секция очистки кеша
        cache_group = QGroupBox("🖮️ Управление кешем")
        cache_layout = QVBoxLayout(cache_group)
        
        # Описание
        cache_label = QLabel("Очистка кеша может помочь решить проблемы с загрузкой и производительностью")
        cache_layout.addWidget(cache_label)
        
        # Кнопки очистки
        buttons_layout = QHBoxLayout()
        
        self.clear_pycache_btn = QPushButton("🖮️ Очистить Python кеш")
        self.clear_pycache_btn.setToolTip("Удалить все __pycache__ папки и .pyc файлы")
        self.clear_pycache_btn.clicked.connect(self.clear_python_cache)
        buttons_layout.addWidget(self.clear_pycache_btn)
        
        self.clear_db_cache_btn = QPushButton("🗄️ Очистить кеш БД")
        self.clear_db_cache_btn.setToolTip("Очистить временные данные в базе данных")
        self.clear_db_cache_btn.clicked.connect(self.clear_db_cache)
        buttons_layout.addWidget(self.clear_db_cache_btn)
        
        self.clear_all_cache_btn = QPushButton("[HOT] Очистить весь кеш")
        self.clear_all_cache_btn.setToolTip("Полная очистка всех временных файлов")
        self.clear_all_cache_btn.clicked.connect(self.clear_all_cache)
        self.clear_all_cache_btn.setStyleSheet("QPushButton { background-color: #ff6b6b; color: white; }")
        buttons_layout.addWidget(self.clear_all_cache_btn)
        
        cache_layout.addLayout(buttons_layout)
        
        # Статистика кеша
        self.cache_stats_label = QLabel("[STATS] Статистика кеша:")
        ...
        self.update_cache_stats_btn = QPushButton("🔄 Обновить статистику")
        self.update_cache_stats_btn.clicked.connect(self.update_cache_stats)
        cache_layout.addWidget(self.update_cache_stats_btn)
        
        layout.addWidget(cache_group)
        layout.addStretch()
        
        # Обновляем статистику при загрузке
        QTimer.singleShot(100, self.update_cache_stats)
        
        return widget
        
    @log_dropdown_change("Режим газа")
    ...
    @log_click("Сохранить настройки")
    @log_action("Сохранение настроек")
    ...
    @log_validation("Валидация настроек")
    ...
    @log_click("Сбросить настройки")
    @log_action("Сброс настроек")
    ...
    def on_test_complete(self, result: dict):
        ...
        if result['success']:
            ...
            msg = f"[ERROR] {result['url']}\nОшибка: {result['error']}\n"
            self.test_results.append(msg)
            self.log_message("RPC тест неудачен", "ERROR")
            
    @log_click("Тестировать все RPC")
    @log_action("Тестирование всех RPC")
    def test_all_rpcs(self):
        ...
        for name, info in stats.items():
            if info['status'] == 'online':
                msg = f"[OK] {name}: {info['response_time_ms']} мс\n"
            else:
                msg = f"[ERROR] {name}: {info.get('error', 'Offline')}\n"
            self.test_results.append(msg)
            
    @log_click("Проверить API ключи")
    @log_api_call("Тест Etherscan API ключей")
    ...
    @log_click("Экспорт настроек")
    @log_action("Экспорт настроек в файл")
    ...
    @log_click("Импорт настроек")
    @log_action("Импорт настроек из файла")
    ...
    @log_click("Очистить Python кеш")
    @log_action("Очистка Python кеша")
    def clear_python_cache(self):
        """Очистка Python кеша (__pycache__ и .pyc файлов)"""
        try:
            # Определяем корневую директорию проекта
            project_dir = pathlib.Path("C:\\Users\\konfu\\Desktop\\Sites\\Experiment\\Experiment1\\WalletSender_MCP  копия\\WalletSender_Modular\\WalletSender_Modular")
            ...
            # Удаляем все .pyc файлы вне __pycache__
            for pyc_file in project_dir.rglob("*.pyc"):
                if pyc_file.is_file() and "__pycache__" not in str(pyc_file):
            ...
            message = f"Очищено: {dirs_removed} папок, {files_removed} файлов ({size_str})"
            self.log_message(message, "SUCCESS")
            logger.info(f"Python кеш очищен: {message}")
            
        except Exception as e:
            logger.error(f"Ошибка очистки Python кеша: {e}")
            self.log_message(f"Ошибка очистки кеша: {e}", "ERROR")
    
    @log_click("Очистить кеш БД")
    @log_action("Очистка кеша базы данных")
    def clear_db_cache(self):
        """Очистка кеша базы данных"""
        try:
            # Очищаем временные таблицы и старые записи
            from ...database.database import get_session
            from datetime import datetime, timedelta
            
            session = get_session()
            
            # Удаляем старые записи из истории (старше 30 дней)
            cutoff_date = datetime.now() - timedelta(days=30)
            
            # TODO: Реализовать очистку старых записей в БД
            # На данный момент просто выполняем VACUUM для SQLite
            session.execute("VACUUM")
            session.commit()
            session.close()
            
            self.update_cache_stats()
            self.log_message("Кеш базы данных очищен", "SUCCESS")
            logger.info("Кеш базы данных очищен")
            
        except Exception as e:
            logger.error(f"Ошибка очистки кеша БД: {e}")
            self.log_message(f"Ошибка очистки кеша БД: {e}", "ERROR")
    
    @log_click("Очистить весь кеш")
    @log_action("Полная очистка кеша")
    def clear_all_cache(self):
        """Полная очистка всего кеша"""
        reply = QMessageBox.question(
            self, "Подтверждение",
            "Вы уверены, что хотите очистить весь кеш?\n"
            "Это может потребовать перезапуска приложения.",
            QMessageBox.Yes | QMessageBox.No
        )
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                # Удаляем .tmp файлы  
                for tmp_file in project_dir.rglob("*.tmp"):
                    if tmp_file.is_file():
                        tmp_file.unlink(missing_ok=True)
                
                self.update_cache_stats()
                self.log_message("Весь кеш успешно очищен", "SUCCESS")
                logger.info("Выполнена полная очистка кеша")
    ...
    @log_click("Обновить статистику кеша")
    def update_cache_stats(self):
        """Обновление статистики кеша"""
        try:
            project_dir = pathlib.Path("C:\\Users\\konfu\\Desktop\\Sites\\Experiment\\Experiment1\\WalletSender_MCP  копия\\WalletSender_Modular\\WalletSender_Modular")
            
            # Подсчитываем размеры кешей
            pycache_size = 0
            pycache_count = 0
            ...
            for pycache_dir in project_dir.rglob("__pycache__"):
                if pycache_dir.is_dir():
                    ...
                    for file in pycache_dir.iterdir():
                        if file.is_file():
                            pycache_size += file.stat().st_size
            
            # Подсчитываем .pyc файлы
            pyc_size = 0
            pyc_count = 0
            ...
            db_size = 0
            db_files = [
                project_dir / "wallet_sender.db",
                project_dir / "wallet_sender_store.db"
            ]
            ...
            stats_text = f"""
📂 Python кеш:
  • __pycache__ папок: {pycache_count}
  • .pyc файлов: {pyc_count}
  • Общий размер: {format_size(pycache_size + pyc_size)}

🗄️ База данных:
  • Размер БД: {format_size(db_size)}

[INFO] Лог файлы:
  • Количество: {log_count}
  • Размер: {format_size(log_size)}

[STATS] Общий размер кеша: {format_size(pycache_size + pyc_size + log_size)}
            """.strip()
            
            self.cache_stats_text.setPlainText(stats_text)
            
        except Exception as e:
            logger.error(f"Ошибка обновления статистики кеша: {e}")
            self.cache_stats_text.setPlainText(f"Ошибка получения статистики: {e}")
