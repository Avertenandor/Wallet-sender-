...
from ...utils.logger_enhanced import (
    ...
    log_table_action, log_file_operation, log_radio_change
)
...
class MassDistributionTab(BaseTab):
    ...
    def init_ui(self):
        ...
        self.token_combo.currentTextChanged.connect(
            lambda t: log_dropdown_change("Токен для рассылки")(lambda: None)()
        )
        ...
        self.amount_input.valueChanged.connect(
            lambda v: log_spinbox_change("Сумма на адрес")(lambda: None)()
        )
        ...
        gas_layout.addWidget(QLabel("Цена газа:"))
    ...
    @log_click("Импорт из файла")
    @log_file_operation("Импорт адресов")
    ...
    @log_click("Начать рассылку")
    @log_action("Массовая рассылка токенов")
    def start_distribution(self):
        ...
        self.log(f"[START] Начинаем массовую рассылку {token_type} (Слот {self.slot_number})", "SUCCESS")
        self.log(f"[STATS] Параметры: {amount} токенов, {cycles} циклов, интервал {interval}с", "INFO")
        self.log(f"[INFO] Адресов в списке: {len(self.addresses)}", "INFO")
    ...
    def _distribution_worker(self, token_type: str, token_address: str, amount: float, cycles: int, interval: int):
        ...
        try:
            ...
            self.log(f"[FINISH] Рассылка завершена! Отправлено: {total_sent}, ошибок: {total_errors}", "SUCCESS")
    ...
    def connect_wallet(self, checked=False):
        ...
        if not self.web3 or not self.web3.is_connected():
            self.log("[ERROR] Нет подключения к BSC", "ERROR")
            return
            
        wallet_input = self.wallet_input.toPlainText().strip()
        
        if not wallet_input:
            self.log("[ERROR] Введите SEED фразу или приватный ключ", "ERROR")
            return
        ...
        try:
            ...
            if ' ' in wallet_input:  # SEED фраза
                ...
                if not created:
                    self.log("[ERROR] Не удалось создать кошелек из SEED фразы. Введите приватный ключ или установите зависимости: mnemonic, bip_utils.", "ERROR")
                    return
            else:  # Приватный ключ
            ...
            self.log(f"[ERROR] Ошибка подключения: {str(e)}", "ERROR")
        ...
        try:
            ...
            try:
                ...
                if not contract_code or contract_code == b'':
                    self.log(f"[ERROR] Контракт PLEX ONE не найден по адресу {plex_checksum}", "ERROR")
                    plex_formatted = 0
                else:
            ...
            try:
                ...
                if not contract_code or contract_code == b'':
                    self.log(f"[ERROR] Контракт USDT не найден по адресу {usdt_checksum}", "ERROR")
                    usdt_formatted = 0
                else:
    ...
    def _send_bnb(self, to_address: str, amount: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Используем кастомную цену газа из интерфейса
            custom_gas_price_gwei = self.gas_price_input.value()
            gas_price = self.web3.to_wei(custom_gas_price_gwei, 'gwei')
