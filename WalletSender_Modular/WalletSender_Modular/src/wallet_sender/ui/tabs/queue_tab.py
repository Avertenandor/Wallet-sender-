...
class QueueTab(BaseTab):
    ...
    def init_ui(self):
        ...
        header_label = QLabel("[INFO] Управление очередью операций")
    ...
    def _create_active_queue_panel(self) -> QWidget:
        ...
        self.export_queue_btn = QPushButton("[SEND] Экспорт очереди")
    ...
    def _create_completed_tasks_panel(self) -> QWidget:
        ...
        # Заголовок
        header_layout = QHBoxLayout()
        header_layout.addWidget(QLabel("[OK] Завершенные задачи"))
        ...
        self.export_completed_btn = QPushButton("[STATS] Экспорт отчета")
    ...
    def _create_control_panel(self) -> QGroupBox:
        ...
        # Кнопки управления
        self.start_btn = QPushButton("[PLAY] Запустить обработку")
        ...
        self.resume_btn = QPushButton("[PLAY] Продолжить")
    ...
    def start_processing(self):
        ...
        self.log("[PLAY] Запущена обработка очереди", "SUCCESS")
    ...
    def resume_processing(self):
        ...
        if self.is_processing:
            ...
            self.pause_btn.setEnabled(True)
            self.resume_btn.setEnabled(False)
            self.log("[PLAY] Обработка возобновлена", "SUCCESS")
    ...
    def _processing_worker(self):
        ...
        try:
            ...
            self._log_execution(f"[ERROR] Критическая ошибка: {str(e)}")
    ...
    def _execute_task(self, task: Dict):
        ...
        try:
            ...
            self._log_execution(f"[OK] Задача выполнена: {task['id']}")
            ...
            if self.auto_retry.isChecked() and task['retries'] < self.max_retries.value():
                ...
                self._log_execution(f"[ERROR] Ошибка задачи {task['id']}: {str(e)}")
    ...
    def _show_active_context_menu(self, position):
        ...
        view_details = menu.addAction("[SEARCH] Подробности")
        ...
        menu.addSeparator()
        cancel_task = menu.addAction("[ERROR] Отменить задачу")
...
{json.dumps(task['params'], indent=2, ensure_ascii=False)}
    ...
    def export_queue(self):
        ...
        if path:
            try:
                ...
                self.log(f"[OK] Экспортировано {len(export_data)} задач", "SUCCESS")
    ...
    def import_queue(self):
        ...
        try:
            ...
            self.log(f"[OK] Импортировано {imported} задач", "SUCCESS")
    ...
    def export_completed_report(self):
        ...
        if path:
            try:
                ...
                self.log(f"[OK] Экспортирован отчет по {self.completed_table.rowCount()} задачам", "SUCCESS")
