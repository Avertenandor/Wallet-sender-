...
class SearchTab(BaseTab):
    ...
    def _create_search_params_group(self) -> QGroupBox:
        ...
        self.search_btn = QPushButton("[SEARCH] Начать поиск")
        ...
        self.export_btn = QPushButton("[STATS] Экспорт в CSV")
    ...
    def start_search(self):
        ...
        self.log(f"[SEARCH] Начат поиск транзакций для адреса: {address[:8]}...{address[-6:]}", "INFO")
    ...
    def _start_async_search(self, address: str):
        ...
        async def search_coroutine():
            try:
                ...
                self._log_to_search(f"[ERROR] Ошибка: {str(e)}")
    ...
    def _on_search_complete(self, results: List[Dict]):
        ...
        self._log_to_search(f"[OK] Поиск завершен. Найдено: {len(results)} транзакций")
    ...
    def _update_results_table(self, transactions: List[Dict]):
        ...
        try:
            ...
            for tx in transactions[:1000]:  # Ограничиваем отображение
                ...
                status_item = QTableWidgetItem("[ERROR] Ошибка" if is_error else "[OK] Успешно")
                if is_error:
    ...
    def _on_error(self, error_msg: str):
        ...
        self.log(f"[ERROR] Ошибка: {error_msg}", "ERROR")
    ...
    def _show_context_menu(self, position):
        ...
        copy_hash = menu.addAction("[INFO] Копировать hash")
        copy_from = menu.addAction("[INFO] Копировать отправителя")
        copy_to = menu.addAction("[INFO] Копировать получателя")
        menu.addSeparator()
        open_bscscan = menu.addAction("[WEB] Открыть в BscScan")
        menu.addSeparator()
    ...
    def export_results(self):
        ...
        if path:
            try:
                ...
                self.log(f"[OK] Экспортировано {len(self.search_results)} транзакций", "SUCCESS")
