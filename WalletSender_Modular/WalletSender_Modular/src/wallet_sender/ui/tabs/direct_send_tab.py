...
from ...utils.logger_enhanced import (
    log_action, log_click, log_input_change, log_transaction,
    log_network_action, set_ui_log_handler
)
...
class DirectSendTab(BaseTab):
    ...
    def init_ui(self):
        ...
        self.send_btn = QPushButton("[START] Отправить")
        ...
        self.cancel_btn = QPushButton("[ERROR] Отменить")
    ...
    def _on_token_changed(self, token):
        ...
        self.log(f"Выбран токен: {token}", "INFO")
        
    @log_click("Подключить кошелек")
    @log_action("Подключение кошелька DirectSend")
    ...
    @log_action("Отключение кошелька DirectSend")
    ...
    @log_click("Обновить баланс")
    @log_network_action("Обновление баланса")
    ...
    @log_click("Отправить транзакцию")
    @log_transaction
    ...
    def _on_transaction_complete(self, job_id: str, success: bool, result: Any):
        ...
        if success:
            ...
            self.log(f"[OK] Транзакция завершена: {tx_hash}", "SUCCESS")
            ...
            self.log(f"[ERROR] Ошибка отправки: {error_msg}", "ERROR")
        ...
        if not self.current_jobs:
            self.send_btn.setEnabled(True)
            self.send_btn.setText("[START] Отправить")
            self.cancel_btn.setEnabled(False)
    
    @log_click("Отменить задачи")
    @log_action("Отмена текущих задач")
    ...
    @log_network_action("Отправка BNB")
    ...
    @log_network_action("Отправка токена")
