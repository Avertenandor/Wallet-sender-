...
class FoundTxTab(BaseTab):
    ...
    def init_ui(self):
        ...
        # Заголовок
        header = QGroupBox("[SEARCH] Найденные транзакции")
    ...
    def show_context_menu(self, position):
        ...
        # Действия
        copy_hash_action = menu.addAction("[INFO] Копировать TX Hash")
        copy_from_action = menu.addAction("[INFO] Копировать адрес отправителя")
        copy_to_action = menu.addAction("[INFO] Копировать адрес получателя")
        menu.addSeparator()
        view_on_bscscan_action = menu.addAction("[CONNECT] Открыть в BscScan")
        menu.addSeparator()
        select_all_action = menu.addAction("[OK] Выбрать все")
        deselect_all_action = menu.addAction("[ERROR] Снять выделение")
