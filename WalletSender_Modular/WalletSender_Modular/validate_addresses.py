...
def validate_and_normalize_addresses(file_path: str):
    ...
    print(f"[STATS] Прочитано строк: {len(lines)}")
    ...
    print(f"[OK] Валидных адресов: {len(addresses)}")
    print(f"[ERROR] Невалидных адресов: {len(invalid)}")
    print(f"[WARN]  Дубликатов: {len(duplicates)}")
    ...
    if found_contracts:
        print("\n[WARN]  ОБНАРУЖЕНЫ КОНТРАКТЫ ТОКЕНОВ:")
    ...
    if addresses:
        ...
        print(f"\n[OK] Нормализованные адреса сохранены в:")
        print(f"   {output_file}")
        ...
        if eoa_addresses:
            ...
            print(f"\n[OK] EOA адреса (без контрактов) сохранены в:")
            print(f"   {eoa_file}")
    ...
    if found_contracts:
        print("[WARN]  Удалите адреса контрактов токенов из списка рассылки")
    
    if duplicates:
        print("[WARN]  Используйте нормализованный файл без дубликатов")
    
    if not invalid and not duplicates and not found_contracts:
        print("[OK] Все адреса валидны и готовы к использованию!")
